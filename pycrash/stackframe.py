...
class StackFrame(object):
    ...
    def toXML(self):
        ...
        frame_element = etree.Element("frame", argcount=str(self.getArgCount()),
                                       size=str(self.getStackSize()),
                                       filename= self.getFileName(),
                                       line = str(self.getLineNumber()))
...
#        strXML += " line=\"" +  str(self.getLineNumber()) + "\">\n"
        ...
        for item in self.getVarNames():
            ...
            try: #the variable can be undefined
                ...
                varValue = "(undefined)"
                varType = "(undefined)"
            var_element = etree.Element("var", name = item,
                                               type = varType)
            var_element.text = varValue
...
#            strXML += " type=\"" + varType + "\">" + varValue
            ...
            if isInstance(var): #Add all object attributes
                ...
                for attr in attrKeys:
                    ...
                    attr_element = etree.Element("attr", name=attr,
                                                         type=varType)
                    attr_element.text = str(var.__dict__[attr])
...
#                    strXML += attrValue + "</attr>"
                    var_element.append(attr_element)
#               strXML += "\n\t\t\t</var>\n"
#           else:
#               strXML += "</var>\n"

            frame_element.append(var_element)            
#        strXML += "\t\t</frame>\n"
#        return strXML
